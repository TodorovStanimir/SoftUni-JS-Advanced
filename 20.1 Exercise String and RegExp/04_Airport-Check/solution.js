function solve() {
    ...
    const patternName = / [A-Z][A-Za-z]*-[A-Z][A-Za-z]*( |\.-[A-Z][A-Za-z]* )/g;
    const patternAirport = / [A-Z]{3}\/[A-Z]{3} /g;
    const patternFly = / [A-Z]{1,3}[\d]{1,5} /g;
    const patternCompany = /\- [A-Z]{1}[a-z]*[*]{1}[A-Z]{1}[a-z]* /g;
    let firstPart = inputElem.value.substr(0, inputElem.value.lastIndexOf(','));
    let secondPart = inputElem.value.substr(inputElem.value.lastIndexOf(',')+1, inputElem.value.length);
    console.log(`first-${firstPart}'`, `second-${secondPart}'`)
